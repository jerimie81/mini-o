...
class MiniOMcpClient(private val endpoint: String = "http://127.0.0.1:8000/api/mcp") {
    ...
    fun request(method: String, params: String = "{}"): String {
        ...
        val request = HttpRequest.newBuilder(URI.create(endpoint)).header("content-type", "application/json").header("MCP-Protocol-Version", "2025-11-25").POST(HttpRequest.BodyPublishers.ofString(body)).build()
        return http.send(request, HttpResponse.BodyHandlers.ofString()).body()
    }
    fun initialize() = request("initialize", "{\"protocolVersion\":\"2025-11-25\",\"capabilities\":{}}")
    fun listTools() = request("tools/list")
    fun sendBuildContext(label: String, text: String) = request("context/submit", "{\"client_id\":\"android-studio\",\"items\":[{\"kind\":\"diagnostic\",\"label\":${quote(label)},\"text\":${quote(text)}}]}")
